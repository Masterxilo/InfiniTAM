...
Kinect2Engine::Kinect2Engine(const char *calibFilename) : ImageSourceEngine(calibFilename)
{
	...
	if (data->kinectSensor)
	{
        ...
        // color
        IColorFrameSource* s = NULL;
        hr = data->kinectSensor->Open();
        if (SUCCEEDED(hr))
            hr = data->kinectSensor->get_ColorFrameSource(&s);
        
        if (SUCCEEDED(hr))
            hr = s->OpenReader(&data->colorFrameReader);
...
void Kinect2Engine::getImages(ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage)
{
	...
	{
        IColorFrame* cf = 0;
        HRESULT hr = data->colorFrameReader->AcquireLatestFrame(&cf);
        if (SUCCEEDED(hr))
        {
            ColorImageFormat cif;
            cf->get_RawColorImageFormat(&cif);

            IFrameDescription *d;
            cf->get_FrameDescription(&d);
            Vector2i dim;
            d->get_Width(&dim.x);
            d->get_Height(&dim.y);
            SafeRelease(d);
            if (imageSize_rgb == dim) {
                hr = cf->CopyConvertedFrameDataToArray(
                    rgbImage->dataSize * sizeof(Vector4u), (BYTE*)rgb, 
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				for (int i = 0; i < imageSize_d.x * imageSize_d.y; i++)
				{
                    ...
                    depthPix == 0 ? -1 : depthPix;
                    depth[i] = depthPix >> 2;// throw away some bits ;//depthPix == 0 ? -1.0f : (float)depthPix / 1000.0f;
				}
			}
		}

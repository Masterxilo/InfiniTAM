...
struct TrackingLevel {
    ...
    TrackingLevel(int numberOfIterations, TrackerIterationType iterationType, float distanceThreshold) :
        ...
        depth = new ITMFloatImage(Vector2i(1, 1));
    }
};
...
static __managed__ AccuCell accu;
    ...
    Matrix4f approxInvPose)
{
    ...
    {
        isValidPoint = computePerPointGH_Depth_Ab<iterationType>(A, b, x, y,
...
AccuCell ComputeGandH(Matrix4f T_g_k_estimate) {
    ...
    ::viewIntrinsics = viewIntrinsics;
    ::viewImageSize = viewImageSize;
    ::distThresh = currentTrackingLevel->distanceThreshold;
#define iteration(it) \
			it: LAUNCH_KERNEL(depthTrackerOneLevel_g_rt_device_main<it>, gridSize, blockSize, depth, T_g_k_estimate);
...
void ITMDepthTrackerOld::TrackCamera(ITMTrackingState *trackingState, const ITMView *view)
{

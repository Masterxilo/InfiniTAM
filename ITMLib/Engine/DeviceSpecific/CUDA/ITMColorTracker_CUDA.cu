...
void ITMColorTracker_CUDA::F_oneLevel(float *f, const ITMPose& pose)
{
    // Aliasing
    int noTotalPoints = trackingState->pointCloud->noTotalPoints;
    Vector4f projParams = projParamsForLevelId(levelId);
...
void ITMColorTracker_CUDA::G_oneLevel(float *gradient, float *hessian, const ITMPose& pose) const
{
	int noTotalPoints = trackingState->pointCloud->noTotalPoints;
    Vector4f projParams = projParamsForLevelId(levelId);
	Matrix4f M = pose.GetM();

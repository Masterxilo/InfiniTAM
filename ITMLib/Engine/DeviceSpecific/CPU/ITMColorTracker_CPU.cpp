...
void ITMColorTracker_CPU::F_oneLevel(float *f, const ITMPose& pose)
{
    // Aliasing
...
void ITMColorTracker_CPU::G_oneLevel(float *gradient, float *hessian, const ITMPose& pose) const
{
    // Aliasing
    int noTotalPoints = trackingState->pointCloud->noTotalPoints;
    Vector4f projParams = projParamsForLevelId(levelId);
